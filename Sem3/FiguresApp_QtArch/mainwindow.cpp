...
{  
        ...
        //QPixmap Hexlepix(":/images/triangle.png");
        QAction* A_createHexagramm = toolbar->addAction(QIcon(trianglepix), "Треугольник");
        connect(A_createHexagramm, SIGNAL(triggered()), this, SLOT(slotCreateHexagramm()));
...
void MainWindow::slotCreateHexagramm()
{
    QDialog* dialog = new QDialog();
    dialog->setWindowTitle("Гексаграмма");
    QFormLayout* formLayout = new QFormLayout();
    QSpinBox* X1 = new QSpinBox();
    X1->setMinimum(0);
    X1->setMaximum(1000);
    ...
    formLayout->addRow("x2:", X2);
    formLayout->addRow("y2:", Y2);
    formLayout->addRow("y5:", Y5);
    formLayout->addRow("x3:", X3);
    formLayout->addRow("y3:", Y3);
    formLayout->addRow("y6:", Y6);
    QDialogButtonBox* buttonBox = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel);
    connect(buttonBox, SIGNAL(accepted()), dialog, SLOT(accept()));
    connect(buttonBox, SIGNAL(rejected()), dialog, SLOT(reject()));
    formLayout->addWidget(buttonBox);
    ...
    if(result == QDialog::Accepted)
    {
        try
        {
            HexagrammItem* hexagrammItem = new HexagrammItem(Hexagramm(X1->value(), Y1->value(),Y4->value(),
                                                                   X2->value(), Y2->value(),Y5->value(),
                                                                   X3->value(), Y3->value(),Y6->value()), QPoint(0, 0), color);
            scene->addItem(hexagrammItem);
        }
        catch(...) {}
    }
    delete X1;
