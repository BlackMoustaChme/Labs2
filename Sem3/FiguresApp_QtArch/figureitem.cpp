...
void FigureItem::updateInfo()
{
   ...
   QString InfoFigure = QString("Площадь: %1\nПериметр: %2").arg(figure->CalcSquare()).arg(figure->CalcPerimeter());
...
HexagrammItem::HexagrammItem(Hexagramm hexagramm, QPoint position, QColor color): FigureItem(position, color)
{
    figure = new Hexagramm(hexagramm);
    setPaint();
    updateInfo();
}


void HexagrammItem::setPaint()
{
    path = QPainterPath();
    int x1, y1, y4, x2, y2, y5, x3, y3, y6;
    ((Hexagramm*)figure)->getCoor(x1, y1, y4, x2, y2, y5, x3, y3, y6);
    path.addPolygon(QPolygon(QList<QPoint>{QPoint(x1, y1), QPoint(x2, y2), QPoint(x3, y3)}));
    path.addPolygon(QPolygon(QList<QPoint>{QPoint(x1, y4), QPoint(x2, y5), QPoint(x3, y6)}));
    bounding = QRectF(QPolygon(QList<QPoint>{QPoint(x1, y1), QPoint(x2, y2), QPoint(x3, y3)}).boundingRect());
    //bounding = QRectF(QPolygon(QList<QPoint>{QPoint(x1, y4), QPoint(x2, y5), QPoint(x3, y6)}).boundingRect());
}


HexagrammItem::~HexagrammItem()
{
    delete (Hexagramm*)figure;
}
